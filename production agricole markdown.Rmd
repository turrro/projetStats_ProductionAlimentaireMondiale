...
plot(ancova_rdt1.lm)
```
Tout d'abord, on peut voir que notre modèle a un p-value significative, cela veut dire qu'il est plus intéressant que le modèle nul, et on peut voir que presque tous les coefficients sont significativement non nuls. Cependant, au vu des graphes de diagnostic, certaines hypothèses ne sont pas vérifiées. Premièrement, il semble que les erreurs ne soient pas centrées, on peut voir de gros écarts sur leurs valeurs. De même, on peut avoir quelques doutes concernant l'homoscédasticité des erreurs. Enfin, l'hypothèse de normalité des erreurs est aussi à reconsidérer. Peut-être qu'on peut donc affiner notre modèle.
