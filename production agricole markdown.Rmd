...
```{r data}
agri = read.table("Yield.csv", sep= ' ')
head(agri)
agripays = read.table("Yieldbycountry.csv", sep = ' ')
head(agripays)
```
...
table(agri$Item)
```
Ainsi on peut voir que certaines cultures sont sur-représentées dans la production agricole et que d'autres sont sous-représentées. Peut-être qu'on peut justement essayer de séparer ces cultures en deux catégories, en fixant un certain seuil par exemple ?

```{r efficience pesticides}
ggplot(agri, aes(y=yield, x=pest, color=Item)) + geom_point() + theme_bw()
ggplot(agri, aes(x=yield, y=pest, color=Item)) + geom_point() + theme_bw()
```